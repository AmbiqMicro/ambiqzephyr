...
static bool check_i2s_data(uint32_t rxtx_sample_num, void *rx_databuf)
{
	...
	for (i = 0; i < (rxtx_sample_num - index_0); i++) {
		...
		if (rx_databuf_16[idx] != (0xCD00 | ((i % SAMPLES_PER_BLOCK) & 0xFF))) {
			printk("idx %d 0x%x buf[%d] = 0x%x 0x%x", index_0,
			       (idx > 0) ? rx_databuf_16[idx - 1] : 0, i, rx_databuf_16[idx],
			       ((idx + 1) < rxtx_sample_num) ? rx_databuf_16[idx + 1] : 0);
			return false;
		}
	}
#else
	...
	for (i = 0; i < (rxtx_sample_num - index_0); i++) {
		...
		if (rx_databuf_32[idx] != (0xCD0000 | ((i % SAMPLES_PER_BLOCK) & 0xFF))) {
			printk("idx %d 0x%x buf[%d] = 0x%x 0x%x", index_0,
			       (idx > 0) ? rx_databuf_32[idx - 1] : 0, i, rx_databuf_32[idx],
			       ((idx + 1) < rxtx_sample_num) ? rx_databuf_32[idx + 1] : 0);
			return false;
		}
	}
#endif
...
int main(void)
{
...
#else
	i2s_config_param.format = I2S_FMT_DATA_FORMAT_PCM_SHORT;
#endif
	...
	if (ret < 0) {
		LOG_ERR("Send I2S RX trigger DROP failed: %d", ret);
		return 0;
	}
#endif
