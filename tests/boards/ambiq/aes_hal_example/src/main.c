...
static bool run_ctr_test(void)
{
	...
	/*
	 * Key, nonce and message are RFC 3686 test vector #1, but that vector
	 * starts its counter at 1 and the Zephyr split-counter API owns the
	 * counter field, which starts at 0. Only the 12-byte IV portion is
	 * passed, so the expected ciphertext is recomputed for that block.
	 */
	...
	static const uint8_t ct_exp[16] = {
		0x14, 0x7E, 0x4F, 0xE7, 0x8B, 0xBE, 0x75, 0x59,
		0xC1, 0x53, 0x20, 0xA5, 0x37, 0x9F, 0xDD, 0x50,
	};
