...
void pm_state_exit_post_ops(enum pm_state state, uint8_t substate_id)
{
	...
	switch (state) {
	case PM_STATE_SUSPEND_TO_IDLE:
		/*Place holder, nothing is needed after soc is wake up.*/
		break;
	case PM_STATE_SUSPEND_TO_RAM:
		/*Place holder, nothing is needed after soc is wake up.*/
		break;
...
{
    /* Enable flash. 
	 * Currently all flash area is powered on, but we should only enable the used flash area and put unused flash in power down mode.
	 */ 
    ...
    /* Enable SRAM. 
	 * Currently all SRAM area is powered on, but we should only enable the used ram area and put unused ram in power down mode.
	 */ 
