...
static int amic_ambiq_audadc_init(const struct device *dev)
{
    ...
    //
    if ( AM_HAL_STATUS_SUCCESS != am_hal_audadc_initialize(data->inst_idx, &data->audadc_handler) )
    {
    ...
    // Power on the AUDADC.
    //
    if (AM_HAL_STATUS_SUCCESS != am_hal_audadc_power_control(data->audadc_handler,
                                                          AM_HAL_SYSCTRL_WAKE,
                                                          false) )
    {
...
static int amic_ambiq_audadc_configure(const struct device *dev, struct amic_cfg *dev_config)
{
    ...
    if (AM_HAL_STATUS_SUCCESS != am_hal_audadc_configure(data->audadc_handler, &data->audadc_cfg))
    {
    ...
    data->dma_cfg.ui32TargetAddressReverse =
        data->dma_cfg.ui32TargetAddress + (data->dma_cfg.ui32SampleCount * sizeof(uint32_t));

    if (AM_HAL_STATUS_SUCCESS != am_hal_audadc_configure_dma(data->audadc_handler, &(data->dma_cfg)))
    {
        LOG_ERR("Error - configuring AUDADC DMA failed.\n");
    }

    am_hal_audadc_interrupt_clear(data->audadc_handler, AM_HAL_AUDADC_INT_DERR | AM_HAL_AUDADC_INT_DCMP);
    am_hal_audadc_interrupt_enable(data->audadc_handler, AM_HAL_AUDADC_INT_DERR | AM_HAL_AUDADC_INT_DCMP);
    ...
    gain_config.ui32HGADELTA = (uint32_t)((float)(CH_A1_GAIN_DB * 2 + 12)) - gain_config.ui32LGA; // HGDelta = 12
    ...
    //
    int ret = am_hal_audadc_slot_dc_offset_calculate(data->audadc_handler, 2, &(data->offset_cal_array));
			       ...
			       size_t *size, int32_t timeout)
{
	...
	if (ret != 0) {
		...
		uint32_t *audadc_data_buf = (uint32_t *)am_hal_audadc_dma_get_buffer(data->audadc_handler);
    ...
    if (AM_HAL_STATUS_SUCCESS != am_hal_audadc_interrupt_clear(data->audadc_handler, 0xFFFFFFFF))
    {
...
#define AMBIQ_AUDADC_DEFINE(n)                                                                        \
	...
	static am_hal_audadc_sample_t audadc_lg_sample_buf##n[DT_INST_PROP_OR(n, audadc_buffer_size, 1536)];     \
...
//	__attribute__((section(DT_INST_PROP_OR(n, audadc_buffer_location, "SRAM_NO_CACHE"))))                 \
