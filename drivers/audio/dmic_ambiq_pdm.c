...
static __aligned(32) struct {
	__aligned(32) uint32_t buf[CONFIG_PDM_DMA_TCB_BUFFER_SIZE];                     // CONFIG_PDM_DMA_TCB_BUFFER_SIZE should be 2 x block_size
} pdm_dma_tcb_buf[DT_NUM_INST_STATUS_OKAY(DT_DRV_COMPAT)] __attribute__((__section__(".dtcm_data")));
...
static int dmic_ambiq_pdm_configure(const struct device *dev,
				   struct dmic_cfg *dev_config)
{
	struct dmic_ambiq_pdm_data *data = dev->data;
	const struct dmic_ambiq_pdm_cfg *config = dev->config;
    ...
    am_hal_pdm_interrupt_clear(data->pdm_handler, (AM_HAL_PDM_INT_DERR
                                            | AM_HAL_PDM_INT_DCMP
                                            | AM_HAL_PDM_INT_UNDFL
                                            | AM_HAL_PDM_INT_OVF));
    am_hal_pdm_interrupt_enable(data->pdm_handler, (AM_HAL_PDM_INT_DERR
                                            | AM_HAL_PDM_INT_DCMP
                                            | AM_HAL_PDM_INT_UNDFL
                                            | AM_HAL_PDM_INT_OVF));
    ...
    data->pdm_transfer.ui32TargetAddrReverse = data->pdm_transfer.ui32TargetAddr + data->pdm_transfer.ui32TotalCount;
...
static int dmic_ambiq_pdm_trigger(const struct device *dev,
				 enum dmic_trigger cmd)
{
	...
	case DMIC_TRIGGER_START:
		if (data->dmic_state == DMIC_STATE_PAUSED || data->dmic_state == DMIC_STATE_CONFIGURED) {
...
static int dmic_ambiq_pdm_read(const struct device *dev,
			      uint8_t stream,
			      void **buffer, size_t *size, int32_t timeout)
{
	...
	if (ret != 0) {
        ...
        //
        // PDM DMA is 32-bit datawidth for each sample, so we need to invalidate 2x block_size
        //
        am_hal_cachectrl_dcache_invalidate(&(am_hal_cachectrl_range_t){(uint32_t)pdm_data_buf, data->block_size * 2}, false);
...
#define AMBIQ_PDM_DEFINE(n)						                                    \
	...
	DEVICE_DT_INST_DEFINE(n, dmic_ambiq_pdm_init, NULL,		                        \
			 &dmic_ambiq_pdm_data##n, &dmic_ambiq_pdm_cfg##n,                       \
