 ...
 * The EEPROM backup occurs automatically or can be triggered via the control
 * register at I2C address 0x18 (not implemented in this driver).
 */
			     ...
			     void *buf, size_t len)
{
	...
	while (len) {
		...
		if (config->addr_width == 16) {
			...
			ret = i2c_write_read_dt(&config->i2c, addr, 2, pbuf, chunk_len);
		} else {
			addr[0] = offset & 0xFF;
			ret = i2c_write_read_dt(&config->i2c, addr, 1, pbuf, chunk_len);
		}
			      ...
			      const void *buf, size_t len)
{
	...
	while (len) {
		...
		/* Limit chunk to 256 bytes to fit in buffer */
		if (chunk_len > 256) {
			chunk_len = 256;
		}
...
static const struct eeprom_driver_api eeprom_47l16_api = {
...
#define EEPROM_47L16_DEVICE(inst)					\
	static struct eeprom_47l16_data eeprom_47l16_data_##inst;	\
									\
	static const struct eeprom_47l16_config eeprom_47l16_config_##inst = { \
