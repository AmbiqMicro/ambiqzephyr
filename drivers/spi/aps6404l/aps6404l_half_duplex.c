/* aps6404l.c - APS6404L Three-Axis Digital Accelerometers */
...
} g_SPI_SpeedMax[] =
{
	...
	{APS6404L_SPEED_8MHZ,  APS6404L_8MHZ_MAX_BYTES}  //!< Leave this in for PSRAM initialization at 8MHz.
};

uint32_t g_ui32MaxTransSize = 0;

//*****************************************************************************
//
//
//*****************************************************************************
int aps6404l_command_read(const struct device *dev,
							uint8_t reg_addr, 
							uint8_t *pData,
...
int aps6404l_write(const struct device *dev,
	...
	while (ui32NumBytes)
	{
		...
		uint32_t maxSize = APS6404L_PAGE_SIZE - (ui32WriteAddress & (APS6404L_PAGE_SIZE - 1));
...
//
//*****************************************************************************
int aps6404l_read(const struct device *dev,
	...
	while (ui32NumBytes)
	{
		...
		uint32_t maxSize = APS6404L_PAGE_SIZE - (ui32ReadAddress & (APS6404L_PAGE_SIZE - 1));
		uint32_t limit = (maxSize > g_ui32MaxTransSize) ? g_ui32MaxTransSize : maxSize;
		uint32_t size = (ui32NumBytes > limit) ? limit : ui32NumBytes;
...
static int aps6404l_read_id(const struct device *dev, uint8_t *pDeviceID) {
	...
	//
	if ( 0 != aps6404l_command_read(dev, APS6404L_READ_ID, pDeviceID, 5))
	{
		return -EINVAL;
	}
	return 0;
...
#define APS6404L_DEFINE(inst)													\
	...
	static const struct aps6404l_config aps6404l_config_##inst = {				\
		.spec = SPI_DT_SPEC_INST_GET(inst, ( SPI_HALF_DUPLEX | SPI_OP_MODE_MASTER | SPI_TRANSFER_MSB | SPI_WORD_SET(8) | SPI_LINES_SINGLE), 0),	\
	};																			\
																				\
	DEVICE_DT_INST_DEFINE(inst, aps6404l_init, NULL, &aps6404l_data_##inst,		\
			&aps6404l_config_##inst, POST_KERNEL,								\
