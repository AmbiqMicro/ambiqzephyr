...
#ifndef CONFIG_MSPI_DMA_TCB_BUFFER_SIZE
#define CONFIG_MSPI_DMA_TCB_BUFFER_SIZE     1024
#endif
					  ...
					  spi_operation_t operation)
{
	...
	if (IS_ENABLED(CONFIG_SPI_EXTENDED_MODES)) {
		...
		case SPI_LINES_SINGLE:
			line_mode = cfg->mspi_ce ? AM_HAL_MSPI_FLASH_SERIAL_CE1 : AM_HAL_MSPI_FLASH_SERIAL_CE0;
			break;
		case SPI_LINES_DUAL:
			line_mode = cfg->mspi_ce ? AM_HAL_MSPI_FLASH_DUAL_CE1 : AM_HAL_MSPI_FLASH_DUAL_CE0;
			break;
		case SPI_LINES_QUAD:
			line_mode = cfg->mspi_ce ? AM_HAL_MSPI_FLASH_QUAD_CE1 : AM_HAL_MSPI_FLASH_QUAD_CE0;
			break;
		case SPI_LINES_OCTAL:
			line_mode = cfg->mspi_ce ? AM_HAL_MSPI_FLASH_OCTAL_CE0 : AM_HAL_MSPI_FLASH_OCTAL_CE0;
			break;
		default:
...
static int mspi_config(const struct device *dev, const struct spi_config *config)
{
	...
	if (SPI_WORD_SIZE_GET(config->operation) != 8) {
		LOG_ERR("Word reg_size must be %d", SPI_WORD_SIZE);
		return -ENOTSUP;
	}
...
#define AMBIQ_MSPI_DEFINE(n)                                                                       \
    ...
    static struct gpio_dt_spec ce_gpios##n[] = {                                            \
        COND_CODE_1(DT_NODE_HAS_PROP(DT_DRV_INST(n), cs_gpios),                             \
