			       ...
			       struct mspi_callback_context *cb_ctx)
{
	...
	/** user must make sure when cfg_flag = 0 the dummy and instr addr length)
...
#define MSPI_CONFIG(n)                                                                           \
	{                                                                                        \
		...
		.bDQS                  = false,                                                  \
		.ui32SlaveNum          = DT_INST_CHILD_NUM(n),                                   \
		.bSwMultiSlave         = DT_INST_PROP(n, software_multislave),                   \
	}
