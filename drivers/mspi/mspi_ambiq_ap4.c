			       ...
			       struct mspi_callback_context *cb_ctx)
{
	...
	if (!ctx->xfer.async) {
		...
		ret = am_hal_mspi_interrupt_enable(data->mspiHandle, AM_HAL_MSPI_INT_CQUPD | AM_HAL_MSPI_INT_ERR);
		if (ret) {
...
#define AMBIQ_MSPI_DEFINE(n)                                                                       \
	...
	static uint32_t mspi_ambiq_cmdq##n[2560] __attribute__((aligned(16)));                 \
