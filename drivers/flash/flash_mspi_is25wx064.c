...
#define DT_DRV_COMPAT mspi_is25wx064
...
#include "spi_nor.h"
LOG_MODULE_REGISTER(flash_mspi_is25wx064, CONFIG_FLASH_LOG_LEVEL);

#define NOR_WRITE_SIZE                      1
#define NOR_ERASE_VALUE                     0xff

#define IS25WX064_VENDOR_ID                 0x9d5b17

enum is25wx064_dummy_clock {
	is25wx064_DC_8,
	is25wx064_DC_10,
	is25wx064_DC_12,
	is25wx064_DC_14,
...
struct flash_mspi_is25wx064_config {
	uint32_t                            port;
	uint32_t                            mem_size;
...
struct flash_mspi_is25wx064_data {
...
static int is25wx064_get_dummy_clk(uint8_t rxdummy, uint32_t *dummy_clk)
{
	switch (rxdummy) {
	case 8:
		*dummy_clk = is25wx064_DC_8;
		break;
	case 10:
		*dummy_clk = is25wx064_DC_10;
		break;
	case 12:
		*dummy_clk = is25wx064_DC_12;
		break;
	case 14:
		*dummy_clk = is25wx064_DC_14;
		break;
	case 16:
		*dummy_clk = is25wx064_DC_16;
		break;
	case 18:
		*dummy_clk = is25wx064_DC_18;
		break;
	case 20:
		*dummy_clk = is25wx064_DC_20;
		break;
	case 22:
		*dummy_clk = is25wx064_DC_22;
		break;
	default:
		return 1;
	}
...
static int flash_mspi_is25wx064_command_write(const struct device *flash, uint8_t cmd, uint32_t addr,
					    uint16_t addr_len, uint32_t tx_dummy, uint8_t *wdata,
					    uint32_t length)
{
	const struct flash_mspi_is25wx064_config *cfg = flash->config;
	struct flash_mspi_is25wx064_data *data = flash->data;
	int ret;

	data->packet.eDirection       = MSPI_TX;
	data->packet.ui16DeviceInstr  = cmd;
	data->packet.ui32DeviceAddr   = addr;
	data->packet.pui32Buffer      = (uint32_t *)wdata;
	data->packet.ui32NumBytes     = length;

	data->trans.bAsync            = false;
	...
	data->trans.pPacket           = &data->packet;
	data->trans.ui32NumPacket     = 1;
...
static int flash_mspi_is25wx064_command_read(const struct device *flash, uint8_t cmd, uint32_t addr,
...
static int flash_mspi_is25wx064_write_enable(const struct device *flash)
{
	int ret;

	LOG_DBG("Enabling write");
	ret = flash_mspi_is25wx064_command_write(flash, SPI_NOR_CMD_WREN, 0, 0, 0, NULL, 0);

	return ret;
}

static int flash_mspi_is25wx064_write_disable(const struct device *flash)
{
	int ret;

	LOG_DBG("Disabling write");
	ret = flash_mspi_is25wx064_command_write(flash, SPI_NOR_CMD_WRDI, 0, 0, 0, NULL, 0);

	return ret;
}

static void is25wx064_power_reset(const struct device *flash)
{
	uint32_t ui32PinNum = 102;
...
static int flash_mspi_is25wx064_reset(const struct device *flash)
{
	int ret;

	is25wx064_power_reset(flash);

	ret = flash_mspi_is25wx064_write_enable(flash);
	if (ret) {
		return ret;
	}

	LOG_DBG("Return to SPI mode");
	ret = flash_mspi_is25wx064_command_write(flash, 0xFF, 0, 0, 0, NULL, 0);
	if (ret) {
		return ret;
	}

	ret = flash_mspi_is25wx064_write_disable(flash);
	if (ret) {
		return ret;
	}
...
static int flash_mspi_is25wx064_get_vendor_id(const struct device *flash, uint32_t *vendor_id)
{
	struct flash_mspi_is25wx064_data *data = flash->data;
	uint8_t buffer[11];
	int ret;
	...
	LOG_DBG("Reading id");
	/* serial mode */
	ret = flash_mspi_is25wx064_command_read(flash, SPI_NOR_CMD_RDID, 0, 0, 0, buffer, 11);
	*vendor_id = (buffer[0] << 16) | (buffer[1] << 8) | buffer[2];
...
static int flash_mspi_is25wx064_unprotect_sector(const struct device *flash, off_t addr)
{
	int ret;

	LOG_DBG("unprotect sector at 0x%08zx", (ssize_t)addr);

	ret = flash_mspi_is25wx064_command_write(flash, 0x39, addr, 4, 0, NULL, 0);

	return ret;
}

static int flash_mspi_is25wx064_protect_sector(const struct device *flash, off_t addr)
{
	int ret;

	LOG_DBG("protect sector at 0x%08zx", (ssize_t)addr);

	ret = flash_mspi_is25wx064_command_write(flash, 0x36, addr, 4, 0, NULL, 0);

	return ret;
}

static int flash_mspi_is25wx064_erase_sector(const struct device *flash, off_t addr)
{
	int ret;

	LOG_DBG("Erasing sector at 0x%08zx", (ssize_t)addr);

	ret = flash_mspi_is25wx064_command_write(flash, SPI_NOR_CMD_SE, addr, 4, 0, NULL, 0);

	return ret;
}

static int flash_mspi_is25wx064_erase_block(const struct device *flash, off_t addr)
{
	int ret;

	LOG_DBG("Erasing block at 0x%08zx", (ssize_t)addr);

	ret = flash_mspi_is25wx064_command_write(flash, SPI_NOR_CMD_BE, addr, 4, 0, NULL, 0);

	return ret;
}

static int flash_mspi_is25wx064_erase_chip(const struct device *flash)
{
	int ret;

	LOG_DBG("Erasing chip");

	ret = flash_mspi_is25wx064_command_write(flash, SPI_NOR_CMD_CE, 0, 0, 0, NULL, 0);

	return ret;
}

static int flash_mspi_is25wx064_page_program(const struct device *flash, off_t offset, void *wdata,
					   size_t len)
{
	const struct flash_mspi_is25wx064_config *cfg = flash->config;
	struct flash_mspi_is25wx064_data *data = flash->data;
...
static int flash_mspi_is25wx064_busy_wait(const struct device *flash)
{
	const struct flash_mspi_is25wx064_config *cfg = flash->config;
	struct flash_mspi_is25wx064_data *data = flash->data;
	...
	do {
		...
		ret = flash_mspi_is25wx064_command_read(flash, SPI_NOR_CMD_RDSR, 0, 0, rx_dummy,
						      (uint8_t *)&status, 1);
		if (ret) {
...
static int flash_mspi_is25wx064_read(const struct device *flash, off_t offset, void *rdata,
				   size_t len)
{
	const struct flash_mspi_is25wx064_config *cfg = flash->config;
	struct flash_mspi_is25wx064_data *data = flash->data;
...
static int flash_mspi_is25wx064_write(const struct device *flash, off_t offset, const void *wdata,
				    size_t len)
{
	int ret;
	...
	while (len) {
		...
		ret = flash_mspi_is25wx064_page_program(flash, offset, src, i);
		if (ret) {
			return ret;
		}

		ret = flash_mspi_is25wx064_busy_wait(flash);
		if (ret) {
			return ret;
		}
...
static int flash_mspi_is25wx064_erase(const struct device *flash, off_t offset, size_t size)
{
	const struct flash_mspi_is25wx064_config *cfg = flash->config;
	int ret = 0;
	...
	if ((offset == 0) && (size == cfg->mem_size)) {
		...
		ret = flash_mspi_is25wx064_erase_chip(flash);
		if (ret) {
			return ret;
		}
		...
		for (i = 0; i < num_blocks; i++) {
			...
			ret = flash_mspi_is25wx064_unprotect_sector(flash, offset);
			if (ret) {
				return ret;
			}
			...
			ret = flash_mspi_is25wx064_erase_block(flash, offset);
			if (ret) {
				return ret;
			}
		...
		for (i = 0; i < num_sectors; i++) {
			...
			ret = flash_mspi_is25wx064_erase_sector(flash, offset);
			if (ret) {
				return ret;
			}
...
static const struct flash_parameters *flash_mspi_is25wx064_get_parameters(const struct device *flash)
{
	const struct flash_mspi_is25wx064_config *cfg = flash->config;
...
static void flash_mspi_is25wx064_pages_layout(const struct device *flash,
...
static int flash_mspi_is25wx064_init(const struct device *flash)
{
	const struct flash_mspi_is25wx064_config *cfg = flash->config;
	struct flash_mspi_is25wx064_data *data = flash->data;
	uint32_t vendor_id;
	...
	if (flash_mspi_is25wx064_reset(flash)) {
		LOG_ERR("Could not reset Flash/%u", __LINE__);
		return -EIO;
	}

	if (flash_mspi_is25wx064_get_vendor_id(flash, &vendor_id)) {
		LOG_ERR("Could not read vendor id/%u", __LINE__);
//		return -EIO;
	}

	LOG_DBG("Vendor id: 0x%0x", vendor_id);
	if (vendor_id != IS25WX064_VENDOR_ID) {
		LOG_WRN("Vendor ID does not match expected value of 0x%0x/%u", IS25WX064_VENDOR_ID,
			__LINE__);
	}

	if (is25wx064_get_dummy_clk((TIMING_CFG_GET_RX_DUMMY(&cfg->tar_timing_cfg)), &CRB3)) {
		return -ENOTSUP;
	}

	if (flash_mspi_is25wx064_write_enable(flash)) {
		return -EIO;
	}

	if (flash_mspi_is25wx064_command_write(flash, 0x71, 0x3, 1, 0, (uint8_t *)&CRB3, 1)) {
		return -EIO;
	}
	...
	if (flash_mspi_is25wx064_command_write(flash, cmd, 0, 0, 0, NULL, 0)) {
		return -EIO;
	}
...
static int flash_mspi_is25wx064_read_sfdp(const struct device *flash, off_t addr, void *rdata,
					size_t size)
{
	const struct flash_mspi_is25wx064_config *cfg = flash->config;
	struct flash_mspi_is25wx064_data *data = flash->data;
	...
	data->packet.eDirection       = MSPI_RX;
	data->packet.ui16DeviceInstr  = 0x5A;
	data->packet.ui32DeviceAddr   = addr;
	data->packet.pui32Buffer      = (uint32_t *)rdata;
	data->packet.ui32NumBytes     = size;
...
static int flash_mspi_is25wx064_read_jedec_id(const struct device *flash, uint8_t *id)
{
	struct flash_mspi_is25wx064_data *data = flash->data;
...
static int flash_mspi_is25wx064_pm_action(const struct device *flash, enum pm_device_action action)
{
	switch (action) {
...
static const struct flash_driver_api flash_mspi_is25wx064_api = {
	.erase = flash_mspi_is25wx064_erase,
	.write = flash_mspi_is25wx064_write,
	.read = flash_mspi_is25wx064_read,
	.get_parameters = flash_mspi_is25wx064_get_parameters,
#if defined(CONFIG_FLASH_PAGE_LAYOUT)
	.page_layout = flash_mspi_is25wx064_pages_layout,
#endif
#if defined(CONFIG_FLASH_JESD216_API)
	.sfdp_read = flash_mspi_is25wx064_read_sfdp,
	.read_jedec_id = flash_mspi_is25wx064_read_jedec_id,
#endif
};

#define MSPI_DEVICE_CONFIG_SERIAL(n)                                                              \
	{                                                                                         \
		.ui32CENum          = DT_INST_PROP(n, mspi_hardware_ce_num),                      \
		.ui32Freq           = 48000000,                                                   \
		.eIOMode            = MSPI_IO_MODE_OCTAL,                                        \
		.eDataRate          = MSPI_DUAL_DATA_RATE,                                        \
...
#define FLASH_MSPI_is25wx064(n)                                                                     \
	static const struct flash_mspi_is25wx064_config flash_mspi_is25wx064_config_##n = {           \
	...
	static struct flash_mspi_is25wx064_data flash_mspi_is25wx064_data_##n = {                     \
		.lock = Z_SEM_INITIALIZER(flash_mspi_is25wx064_data_##n.lock, 0, 1),                \
	};                                                                                        \
	PM_DEVICE_DT_INST_DEFINE(n, flash_mspi_is25wx064_pm_action);                                \
	DEVICE_DT_INST_DEFINE(n,                                                                  \
			      flash_mspi_is25wx064_init,                                            \
			      PM_DEVICE_DT_INST_GET(n),                                           \
			      &flash_mspi_is25wx064_data_##n,                                       \
			      &flash_mspi_is25wx064_config_##n,                                     \
			      POST_KERNEL,                                                        \
			      CONFIG_FLASH_INIT_PRIORITY,                                         \
			      &flash_mspi_is25wx064_api);

DT_INST_FOREACH_STATUS_OKAY(FLASH_MSPI_is25wx064)
