...
#define CHSC6540_TOUCH_DATA_REG 0x00
...
static void chsc6540_work_handler(struct k_work *work)
{
	...
	uint8_t reg = CHSC6540_TOUCH_DATA_REG;
	int ret = i2c_write_read_dt(&cfg->i2c, &reg, 1, touch_data, CHSC6540_TOUCH_DATA_LEN);
	if (ret < 0) {
	...
	if (num_points >= 2) {
		uint16_t x2 = ((uint16_t)touch_data[9] << 8) | touch_data[10];
		uint16_t y2 = ((uint16_t)touch_data[11] << 8) | touch_data[12];
		// Optionally: report second touch if supported
	}
...
#define CHSC6540_DEFINE(index) \
	...
	DEVICE_DT_INST_DEFINE(index, chsc6540_init, NULL, \
			      &chsc6540_data_##index, &chsc6540_config_##index, POST_KERNEL, \
