...
#ifdef CONFIG_I2C_AMBIQ_HANDLE_CACHE
...
static int i2c_ambiq_read(const struct device *dev, struct i2c_msg *hdr_msg, struct i2c_msg *data_msg,
			  uint16_t addr)
{
	struct i2c_ambiq_data *data = dev->data;
	...
	if (hdr_msg) {
		...
		trans.ui32Instr = (*(uint32_t *)hdr_msg->buf) & (0xFFFFFFFF >> (hdr_msg->len * 8));
#else
		trans.ui64Instr = (*(uint64_t *)hdr_msg->buf) & (0xFFFFFFFF >> (hdr_msg->len * 8));
#endif
...
#define AMBIQ_I2C_DEFINE(n)                                                                        \
	...
	(static uint32_t i2c_ambiq_cmdq##n[DT_PROP_OR(DT_INST_PARENT(n),                           \
						      cmdq_buffer_size, 1024) / 4]                 \
