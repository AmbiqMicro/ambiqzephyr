...
#define AMBIQ_I2C_DEFINE(n)                                                                        \
	...
	{                                                                                          \
		...
		sys_write32((sys_read32(addr) | DT_PHA(DT_INST_PARENT(n), ambiq_pwrcfg, mask)), addr);      \
		k_busy_wait(PWRCTRL_MAX_WAIT_US);                                                  \
		return 0;                                                                          \
	}                                                                                          \
	static void i2c_irq_config_func_##n(void)                                                  \
	{                                                                                          \
		IRQ_CONNECT(DT_IRQN(DT_INST_PARENT(n)), DT_IRQ(DT_INST_PARENT(n), priority), i2c_ambiq_isr,         \
	...
	static const struct i2c_ambiq_config i2c_ambiq_config##n = {                               \
		...
		.inst_idx = (DT_REG_ADDR(DT_INST_PARENT(n)) - IOM0_BASE) / (IOM1_BASE - IOM0_BASE),   \
