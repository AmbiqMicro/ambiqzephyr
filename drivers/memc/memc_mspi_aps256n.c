...
static int memc_mspi_aps256n_reset(const struct device *psram)
{
	...
	ret = memc_mspi_aps256n_command_write(psram, MSPI_PSRAM_DDR_GLOBAL_RESET, 0, 4, 0, 0, (uint8_t *)&pio_buffer, 2);
...
static int memc_mspi_aps256n_enter_hex_mode(const struct device *psram)
{
	...
	ret = memc_mspi_aps256n_command_read(psram, MSPI_PSRAM_DDR_READ_REGISTER, 8, 4, 6,
	...
	ret = memc_mspi_aps256n_command_write(psram, MSPI_PSRAM_DDR_WRITE_REGISTER, 8, 4, 6,
...
#define MEMC_MSPI_APS256N(n)                                                                     \
	static const struct memc_mspi_aps256n_config                                             \
    memc_mspi_aps256n_config_##n = {                                                         \
		.port               = MSPI_PORT(n),                                               \
